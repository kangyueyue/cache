...
import (
	...
	"gitee.com/messizuo/kama-cache-go/singleflight"
...
var (
	...
	groups   = make(map[string]*Group) // 名称到换成组的映射
)
...
// Get 实现Getter接口,缓存为命中时，从DB获取数据
func (f GetterFunc) Get(ctx context.Context, key string) ([]byte, error) {
	return f(ctx, key)
}

// Group 缓存组，管理特定类别数据的缓存
type Group struct {
	name       string              // 组名称
	getter     Getter              // 未命中是回调
	mainCache  *Cache              // 本地缓存
	peers      PeerPicker          // 对等节点
	loader     *singleflight.Group // 加载器,使用单飞机制
...
func WithCacheOption(cacheOpts CacheOptions) GroupOption {
	return func(g *Group) {
...
func NewGroup(name string, cacheBytes int64, getter Getter, opts ...GroupOption) *Group {
	...
	cacheOptions.MaxBytes = cacheBytes

	g := &Group{
		name:      name,
		getter:    getter,
		mainCache: NewCache(cacheOptions),
...
func (g *Group) Set(ctx context.Context, key string, value []byte) error {
	...
	// 检查是否从其他节点同步过的结果
	isPeerRequest := ctx.Value(consts.FromPeer) != nil
...
func (g *Group) load(ctx context.Context, key string) (ByteView, error) {
	...
	if err != nil {
		...
		return ByteView{}, fmt.Errorf("loader.Do error: %v", err)
	}
	...
	// 设置到本地缓存
	if g.expiration > 0 {
		g.mainCache.AddWithExpiration(key, bytes, time.Now().Add(g.expiration))
	} else {
		g.mainCache.Add(key, bytes)
	}
...
func (g *Group) loadData(ctx context.Context, key string) (value ByteView, err error) {
	...
	if g.peers != nil {
		...
		if ok && !self {
			...
			logrus.Warnf("[KamaCache] failed to get from peer, key [%s], err [%v]", key, err) // 远程节点不存在，只能从数据源获取
		}
	...
	if err != nil {
		return ByteView{}, fmt.Errorf("getter.Get error: %v", err)
	}
...
func (g *Group) syncToPeers(ctx context.Context, op string, key string, value []byte) {
	...
	// 创建请求同步上下文
	syncCtx := context.WithValue(context.Background(), consts.FromPeer, true)
...
// DestroyAllGroup 销毁所有group
func DestroyAllGroup() {
	groupsMu.Lock()
	defer groupsMu.Unlock()
